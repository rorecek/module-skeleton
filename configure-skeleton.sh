...
read -p "Package name ($current_directory): " module_name
module_name=${module_name:-$current_directory}

read -p "Package description: " module_description

echo
echo -e "Author: $author_name ($author_username, $author_email)"
echo -e "Package: $module_name <$module_description>"
...
find . -type f -exec sed -i '' -e "s/:module_description/$module_description/g" {} \;
